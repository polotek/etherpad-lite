...
var upsertFunc =  '' +
'CREATE FUNCTION upsert_key(_key VARCHAR, data TEXT, padID VARCHAR, rev INT, last_updated TIMESTAMP WITH TIME ZONE, _network_id VARCHAR) RETURNS VOID AS \n' +
'$$ \n' +
'BEGIN \n' +
'    LOOP \n' +
...
function getColumnData( key, value )
{
  ...
  try
  {
    ...
    if(keyLength > 0 && keyList[0] == 'pad')
    {
      ...
      if(keyLength == 2)
      {
        rev_id = parseInt(jsonValue.head);
        network_id = jsonValue.networkId;
      }
      else if( keyLength >= 4 && keyList[2] == 'revs')
      {
        rev_id = parseInt(keyList[3]);
        timestamp = new Date(jsonValue.meta.timestamp);
      }
    }
