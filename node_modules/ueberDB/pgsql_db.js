...
function getColumnData( key, value )
{
  ...
  try
  {
    ...
    if(keyLength > 0 && keyList[0] == 'pad')
    {
      ...
      if(keyLength == 2)
      {
        rev_id = parseInt(value.head);
        network_id = parseInt(value.networkId);
      }
      else if( keyLength >= 4 && keyList[2] == 'revs')
      {
        rev_id = parseInt(keyList[3]);
        timestamp = new Date(value.meta.timestamp);
      }
    }
...
{
  // If the write buffer still has data, we want to wait until
  // it drains, but it may be empty so set a timeout
  var self = this
    , closeTimeout
    , done = function() {
      // only called once, remove listeners
      clearTimeout(closeTimeout);
      self.db.removeListener('drain', done);
