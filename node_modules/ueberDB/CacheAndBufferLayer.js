  ...
  {      
    ...
    db.wrappedDB.doBulk(operations, function(err)
    {
      operations.forEach(function(op) {
        ...
        msg += op.key + ' - ' + JSON.stringify(op.value) + ' to database';
        db.logger.debug(msg);
      });

      if(err) {
        db.logger.error('Flush operation failed, values will be written with the next flush ', err.message);
        
        //The Flush process failed, so lets try writing that again
        for(var i in operations)
        {
          db.buffer[operations[i].key].dirty = true;
        }
        
        db.flushFailCounter++;
        
        //less than 10 flushes failed so far, that means we can ignor this error
        if(db.flushFailCounter > 10)
        {
          throw err;
        }
      } else {
        db.logger.info('Flush complete');
        process.emit('metric.flush', (new Date()) - flushStart, operations.length);
        db.flushFailCounter = 0;
...
exports.database.prototype.flush = function() {
  var args = Array.prototype.slice.apply(arguments);
  args.unshift(this);
  return flush.apply(this, args);
}

exports.database.prototype.startFlushInterval = function() {
  this.flushInterval = setInterval(flush, this.settings.writeInterval, this);
}

exports.database.prototype.stopFlushInterval = function() {
  if(this.flushInterval) { clearInterval(this.flushInterval); }
}

/**
 * Delete  key from the cache buffer
 */
function cacheDelete(db, key) {
  if(db.buffer[key]) {
    delete db.buffer[key];
    db.bufferLength--;
    db.logger.debug('Deleted ' + key + ' from cache');
    return true;
  } else {
    return false;
  }
 ...
 */
exports.database.prototype.expireKey = function(key, callback) {
  this.gc();
  ...
  if(this.buffer[key]) {
    if(this.isFlushing) {
      // Wait for flush and then delete
      // FIXME: There is the small potential to lose data here
      // if the key is dirty, but not in this flush
      process.once('db:flushing', function() {
        cacheDelete(this, key);
        return callback();
      });
    } else if(this.buffer[key].dirty) {
      // Not flushed, but dirty, so flush it
      // This shouldn't happen because we just called gc
...
/**
 * Evict a set of keys based on a wildcard
 */
exports.database.prototype.expireKeys = function(keyPattern, callback) {
  var self = this
    , re, keys;

  if(typeof keyPattern == 'string') {
    keyPattern = keyPattern.replace('*', '(\\S+)');
    re = new RegExp(keyPattern, 'i')
  } else if(keyPattern instanceof RegExp) {
    re = keyPattern;
  } else {
    return callback( new Error('expireKeys requires a valid regex or glob pattern') );
  }

  keys = Object.keys(this.buffer);
  async.forEach(keys, function(key, callback) {
    if(re.test(key)) {
      self.expireKey(key, callback);
    } else {
      callback();
    }
  },
...
/**
 * Kill the cache buffer
 */
exports.database.prototype.resetBuffer = function resetBuffer() {
  this.buffer = {};
  this.bufferLength = 0;
  this.logger.info('Reset data cache');
}

/**
 * Mercilessly clear the cache
 */
exports.database.prototype.clearCache = function clearCache(callback) {
  var self = this;
  if(self.settings.writeInterval > 0) {
    self.resetBuffer();
  ...
  if(self.isFlushing) {
    // try again after the last flush
    var args = Array.prototype.slice.apply(arguments);
    process.once('db:flushed', function() {
      self.clearCache.apply(self, args);
    });
  } else {
    self.flush(function() {
      self.resetBuffer();
